...
public class FamilyDetailsDaoImpl implements FamilyDetailsDao {
	...
	public String saveFamilyDetails(FamilyDetailsDTO familyDetailsDTO) {
		...
		String insertQuery = "insert into hr_familydetails(parentid,gender,name,relation,dob,adhaarno,qualification,occupation,mobileno,isnominee)"
				+ "values('" + familyDetailsDTO.getParentid() + "','" + familyDetailsDTO.getGender() + "','"
				+ familyDetailsDTO.getName() + "','" + familyDetailsDTO.getRelation() + "','"
				+ familyDetailsDTO.getDob() + "','" + familyDetailsDTO.getAdhaarno() + "','"
				+ familyDetailsDTO.getQualification() + "','" + familyDetailsDTO.getOccupation() + "','"
				+ familyDetailsDTO.getMobileno() + "','"+familyDetailsDTO.getIsnominee()+"')";

		int result = jdbcTemplate.update(insertQuery);
		resultMessage = result > 0 ? HrmsMessageConstants.Family_info_Success : HrmsMessageConstants.Family_info_Fail;
	...
	public String editFamilyDetails(final FamilyDetailsDTO familyDetailsDTO) {
		String resultMessage = "";
		String query = " update hr_familydetails set gender=?, name=?,relation=?,dob=?,adhaarno=?,qualification=?,occupation=?,mobileno=? ,isnominee=? where tranid=? ";
		int result = jdbcTemplate.execute(query, new PreparedStatementCallback<Integer>() {
			...
			public Integer doInPreparedStatement(PreparedStatement ps) throws SQLException {
				...
				ps.setInt(9, familyDetailsDTO.getIsnominee());
				ps.setInt(10, familyDetailsDTO.getTranid());
	...
	public List<FamilyDetailsDTO> getAllFamilyDetails(int parentid) {
		...
		return jdbcTemplate.query("select * from hr_familydetails where parentid=" + parentid,
				new ResultSetExtractor<List<FamilyDetailsDTO>>() {
					...
					public List<FamilyDetailsDTO> extractData(ResultSet rs) throws SQLException, DataAccessException {
						...
						while (rs.next()) {
							...
							dto.setIsnominee(rs.getInt("isnominee"));
							list.add(dto);
