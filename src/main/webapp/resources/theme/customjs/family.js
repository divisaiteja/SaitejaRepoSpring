$(document).ready(function() {
		...
		{
		"ajax" : {
	"url" : "/HRMS/getFamilyInformation?parentid="+ parentid,		
	"dataSrc" : "dataBean",
	"type" : "GET",
},
	"columns" : [
...
function saveFamilyDetails() {
	...
	saveFamilyDetails["mobileno"] = $("#mobileno").val();
	  var checkBox = document.getElementById("isnominee");
	  
	  if(checkBox.checked){
		  saveFamilyDetails["isnominee"] = "1";
	
	  }
	  else {
		saveFamilyDetails["isnominee"]="0";
	}
	//saveFamilyDetails["nominee"] = $("#nominee").val();
	if($("#familytranid").val() == ""){
		$.ajax({
			type : "post",
