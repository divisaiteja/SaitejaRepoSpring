...
public class CadreDaoImpl implements CadreDao {
	...
	public String saveNewCadre(CadreDTO cadreDto) {
		String resultMessage = "";
		String insertQuery="insert into hr_cadre(cadrecode,cadredescription,isactive) values('"+cadreDto.getCadrecode()+ "','" +cadreDto.getCadredescription()+ "',1)";    
		 int result= jt.update(insertQuery); 
			resultMessage = result > 0 ? HrmsMessageConstants.cadre_Save
					: HrmsMessageConstants.cadre_Fail;
			return resultMessage;
	}
