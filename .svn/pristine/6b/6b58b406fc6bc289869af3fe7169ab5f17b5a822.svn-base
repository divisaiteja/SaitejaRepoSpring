...
public class GradeListDaoImpl implements GradeListDao {
	...
	public List<GradeListDTO> getAllGrades() {
		return jt.query(
				"select g.gradeno,g.code,g.description,c.cadredescription,g.isactive from hr_empgrade g join hr_cadre c on g.cadrecode=c.tranid",
				new ResultSetExtractor<List<GradeListDTO>>() {
					@Override
	...
	public String editGrade(GradeListDTO gradelistDTO) {
		...
		String sql = "update hr_empgrade set code='" + gradelistDTO.getCode() + "'," + " description='"
				+ gradelistDTO.getDescription() + "'," + "cadrecode='" + gradelistDTO.getCadrecode() + "',"
				+ "isactive='" + gradelistDTO.getIsactive() + "' where gradeno='" + gradelistDTO.getGradeno() + "'";
	...
	public String saveNewGrade(GradeListDTO gradelistDTO) {
		String resultMessage = "";
		String insertQuery = "insert into hr_empgrade(code,description,cadrecode,isactive) values('"
				+ gradelistDTO.getCode() + "','" + gradelistDTO.getDescription() + "','" + gradelistDTO.getCadrecode()
				+ "',1)";
		int result = jt.update(insertQuery);
		resultMessage = result > 0 ? HrmsMessageConstants.Grade_List_Save : HrmsMessageConstants.Grade_List_Fail;
		return resultMessage;
	...
	public List<GradeListDTO> getGradeByGradeno(int gradeno) {
		return jt.query("select * from hr_empgrade where gradeno = " + gradeno,
				new ResultSetExtractor<List<GradeListDTO>>() {
					@Override
					public List<GradeListDTO> extractData(ResultSet rs) throws SQLException, DataAccessException {
