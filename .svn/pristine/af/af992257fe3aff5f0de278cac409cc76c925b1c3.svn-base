...
public class DepartmentDaoImp implements DepartmentDao {
	...
	public List<DepartmentDTO> getDepartmentByDeptid(int deptId) {
		return jt.query("select * from hr_department where deptid = " + deptId, new ResultSetExtractor<List<DepartmentDTO>>() {
			@Override
			public List<DepartmentDTO> extractData(ResultSet rs) throws SQLException, DataAccessException {
	...
	public String saveNewDepartment(DepartmentDTO deptDto) {
		String resultMessage = "";
			String insertQuery="insert into hr_department(name,shortname,costcenterid,isactive) values('"+deptDto.getName()+ "','" +deptDto.getShortName()+ "'," +deptDto.getCostCenterId()+ ",1)";    
